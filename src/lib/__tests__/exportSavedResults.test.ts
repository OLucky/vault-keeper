...
describe('formatSavedResultsAsText', () => {
  ...
  it('multiple groups produce separate sections', () => {
    const results = [
      makeResult({ categoryId: 'cat-1', categoryName: 'NPCs', tableSetName: 'NPC Gen', savedAt: 2000 }),
      makeResult({ categoryId: 'cat-2', categoryName: 'Weapons', tableSetName: 'Weapon Gen', savedAt: 1000, id: 'w1',
  ...
  it('entries with descriptions show title and description', () => {
    const result = formatSavedResultsAsText([
      makeResult({
        fields: [{ tableName: 'Weapon', entry: { title: 'Blade', description: 'A sharp glass sword' }, tableIndex: 0 }],
      }),
    ])
