...
export function findEntry(entries: Entry[], roll: number): Entry {
  const entry = entries.find((e) => roll >= e.range[0] && roll <= e.range[1]);
  if (!entry) {
...
async function resolveTrigger(
  ...
  if (trigger.tableId && !trigger.tableSet) {
    ...
    if (!table)
      return [
        {
          tableName: trigger.tableId ?? "Unknown",
          entry: { title: "Error" },
          tableIndex: -1,
          error: `Trigger references unknown table ID: ${trigger.tableId}`,
        },
      ];
      ...
      return [
        {
          ...
          entry: { title: "Error" },
          tableIndex: -1,
          error: `Table ID "${trigger.tableId}" not found in ${trigger.tableSet}`,
        },
      ];
...
export async function rerollFieldWithTriggers(
  ...
  if (entry.triggers && entry.triggers.length > 0) {
    ...
    for (const trigger of entry.triggers) {
      const triggerFields = await resolveTrigger(trigger, tableSet, rollFn, fetchTableSet, visited);
