...
- Results that are already saved display a **filled/active bookmark icon** in both the roll view and the session log, indicating their saved state.
  - **Acceptance Criteria:**
    - [x] A save action (e.g., bookmark icon) is visible on the result in the roll view after generating a result.
    - [x] A save action is visible on each entry in the session log sidebar.
    - [x] Clicking the save action adds the result to the Saved Results collection.
    - [x] After saving, the icon changes to a filled/active state indicating the result is saved.
    - [x] The saved state indicator is consistent across roll view and session log (if the same result appears in both).
...
- Notes are free-form text with no required format.
  - **Acceptance Criteria:**
    - [x] After saving a result, the user is given the option to add a note (e.g., an inline text field or a prompt).
    - [x] Notes can be added or edited later from the Saved Results page.
    - [x] Notes are optional — a result can be saved without any note.
    - [x] Notes are persisted alongside the saved result in local storage.
...
- Each saved result displays the generated content, the optional note (if any), and a delete action.
  - **Acceptance Criteria:**
    - [x] A "Saved" link/button is present in the main navigation.
    - [x] The Saved Results page displays all saved results grouped under category and table set headings.
    - [x] Each entry shows the generated result content and its note (if present).
    - [x] If there are no saved results, the page displays an empty state message (e.g., "No saved results yet").
...
- Unsaving a result (clicking the active bookmark icon in the roll view or session log) also removes it from the collection.
  - **Acceptance Criteria:**
    - [x] Each saved result has a delete action on the Saved Results page.
    - [x] Clicking delete shows a confirmation prompt (e.g., "Remove this saved result?").
    - [x] Upon confirmation, the result is removed from the collection and local storage.
    - [x] If the user cancels, the result remains.
    - [x] Clicking an active/filled bookmark icon in the roll view or session log unsaves the result (with confirmation).
    - [x] After removal, the bookmark icon returns to its default/unfilled state.
    - [x] If all results in a group are removed, the group heading is also removed.
...
- Saved results persist independently of the session log — clearing the session log does not affect saved results.
  - **Acceptance Criteria:**
    - [x] Saved results persist across page refreshes and browser tab closures.
    - [x] Clearing the session log does not remove or affect saved results.
    - [x] Notes persist alongside their associated saved results.
...
- The export format is **plain text with headers** — category and table set names as headers, with results (and their notes) listed underneath.
  - **Acceptance Criteria:**
    - [x] A "Copy to Clipboard" button copies the formatted saved results and shows a confirmation message.
    - [x] A "Download" button saves the saved results as a `.txt` file.
    - [x] The exported text includes category and table set headers, result content, and notes (if present).
    - [x] If there are no saved results, the export actions are disabled or hidden.
