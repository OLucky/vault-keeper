...
- [ ] **Slice 10: Dashboard recent rolls**
  - [ ] Wire `recentRolls` from Zustand store to dashboard's recent rolls section. Display last 5 results, newest first. **[Agent: general-purpose]**
  - [ ] Each recent roll renders as a compact `ResultCard` showing table set name and labeled fields. **[Agent: general-purpose]**
  - [ ] Per-field re-rolls on category page update the corresponding entry in `recentRolls` in place (no new entry created). **[Agent: general-purpose]**
  - [ ] Hide recent rolls section entirely when no rolls have been made. **[Agent: general-purpose]**
  - [ ] Write component tests: full roll appears in recent rolls. Per-field re-roll updates existing entry. Max 5 entries enforced. Empty state hidden. **[Agent: general-purpose]**
  - [ ] Verify: Run `npm run dev` — roll on category page, navigate home, see result in recent rolls. Roll 6 times, only 5 most recent shown. **[Agent: general-purpose]**

---

- [ ] **Slice 11: Full sample data + validation error display**
  - [ ] Create complete sample content: 2-3 table sets for NPCs (e.g., "NPC Generator" with race/name/trait/motivation + conditional mutations, "NPC Appearance" with build/clothing/feature). **[Agent: general-purpose]**
  - [ ] Create complete sample content: 2-3 table sets for Weapons/Items (e.g., "Weapon Generator" with type/material/condition, "Trade Goods" with item/value/origin). **[Agent: general-purpose]**
  - [ ] Ensure sample data exercises: different die sizes (d6, d10, d20), multi-result ranges, entries with/without descriptions, conditional tables with triggers, at least one cross-category trigger. **[Agent: general-purpose]**
  - [ ] Wire `ErrorMessage` component into `TableSetEntry`: if a table set fails Zod validation or fetch, show inline error instead of the table set content. Other table sets still function. **[Agent: general-purpose]**
  - [ ] Wire `ErrorMessage` into trigger resolution: if a cross-file trigger fails to resolve, show error for that field but don't break the rest of the result. **[Agent: general-purpose]**
  - [ ] Verify: Run `npm run dev` — browse both categories, roll all table sets, confirm varied results. Intentionally break a JSON file, confirm error message appears inline without crashing the app. **[Agent: general-purpose]**

---

- [ ] **Slice 12: Styling + polish**
  - [ ] Define CSS custom properties in `global.css`: color palette, spacing scale, typography scale, border radius. **[Agent: general-purpose]**
  - [ ] Create CSS Modules for each component: `CategoryCard.module.css`, `TableSetEntry.module.css`, `ResultCard.module.css`, `RerollButton.module.css`, `FilterInput.module.css`, `ErrorMessage.module.css`. **[Agent: general-purpose]**
  - [ ] Style dashboard layout: categories grid/list, recent rolls section. **[Agent: general-purpose]**
  - [ ] Style category page layout: heading, filter input, table set list with inline results. **[Agent: general-purpose]**
  - [ ] Style result cards: labeled fields, re-roll button placement, stacked results spacing. **[Agent: general-purpose]**
  - [ ] Style error messages: distinct but non-alarming inline error display. **[Agent: general-purpose]**
  - [ ] Ensure clean, functional desktop-first UI. Usable (not optimized) on other screen sizes. **[Agent: general-purpose]**
  - [ ] Verify: Run `npm run dev` — visually review all pages and interactions. Clean layout, readable typography, consistent spacing. **[Agent: general-purpose]**
